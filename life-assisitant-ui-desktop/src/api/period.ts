...
export const periodApi = {
  ...
  overview(date?: string): Promise<PeriodOverviewResp> {
    return request.get<unknown, PeriodOverviewResp>('/period/overview', {
      params: date ? { date } : {},
    })
  },
  ...
  calendar(month: string): Promise<PeriodCalendarResp> {
    return request.get<unknown, PeriodCalendarResp>('/period/calendar', { params: { month } })
  },

  /** 区间日记（闭区间，跨度 ≤366 天） */
  listDays(start: string, end: string): Promise<PeriodDaysResp> {
    return request.get<unknown, PeriodDaysResp>('/period/days', { params: { start, end } })
  },

  /** 单日详情；后端无记录时 day 为 null（不是 404） */
  getDay(date: string): Promise<PeriodDayDetailResp> {
    return request.get<unknown, PeriodDayDetailResp>(`/period/days/${date}`)
  },

  /** 写某天日记（整体覆盖：没给的字段一律置为「未记录」） */
  upsertDay(date: string, data: Omit<PeriodDayUpsertReq, 'date'>): Promise<PeriodDayUpsertResp> {
    return request.put<unknown, PeriodDayUpsertResp>(`/period/days/${date}`, { ...data, date })
  },

  /** 删除某天记录（204） */
  deleteDay(date: string): Promise<void> {
    return request.delete<unknown, void>(`/period/days/${date}`)
  },

  /** 周期历史（最近 limit 个） */
  listCycles(limit = 12): Promise<PeriodCyclesResp> {
    return request.get<unknown, PeriodCyclesResp>('/period/cycles', { params: { limit } })
  },

  /** 人工修正周期（period:manage） */
  patchCycle(id: string, data: PeriodCyclePatchReq): Promise<PeriodCyclePatchResp> {
    return request.patch<unknown, PeriodCyclePatchResp>(`/period/cycles/${id}`, data)
  },

  /** 取设置（懒创建，不会 404） */
  getSettings(): Promise<PeriodSettings> {
    return request.get<unknown, PeriodSettings>('/period/settings')
  },

  /** 部分更新设置 */
  patchSettings(data: PeriodSettingsPatchReq): Promise<PeriodSettings> {
    return request.patch<unknown, PeriodSettings>('/period/settings', data)
  },

  /** 引导向导提交（一次性） */
  setup(data: PeriodSetupReq): Promise<PeriodSetupResp> {
    return request.post<unknown, PeriodSetupResp>('/period/setup', data)
  },

  /** 整模块重置（需 confirm:"RESET"，204） */
  reset(): Promise<void> {
    return request.post<unknown, void>('/period/reset', { confirm: 'RESET' })
  },

  /** 周期报告 */
  report(range: '6m' | '12m' = '6m'): Promise<PeriodReportResp> {
    return request.get<unknown, PeriodReportResp>('/period/report', { params: { range } })
  },
}
