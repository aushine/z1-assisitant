 ...
 * 纪念日 / 倒数日 · 展示层工具（桌面端）
 *
 * SYNC-FROM-MOBILE: life-assisitant-ui-mobile/src/utils/anniversary.ts
...
import type { AnniversaryCategory, AnniversaryRepeatRule } from '@/api/types'
 ...
 *    与前端 lucide 的 PascalCase 名不是一套），必须**校验存在性**再传给 <Icon>，
 *    否则图标会整块消失。不存在时回落分类默认图标。
 */
