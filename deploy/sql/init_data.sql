...
--   2. permissions      37 条权限点（11 模块，按真实功能设计，非抽象 CRUD 填充）
--   3. role_permissions admin 37（全量锁定）/ user 27（个人域，权限页可调）
...
-- ---------------------------------------------------------------------
-- 2. permissions（权限点：11 模块 × 实际功能 = 37 条）
...
--    admin：全部 37 点（服务端代码旁路 + 矩阵锁定，改不动也删不得）
--    user ：个人域 27 点（目录推导：排除 user_mgmt / role_mgmt；权限页可调）
-- ---------------------------------------------------------------------
...
--   SELECT COUNT(*) FROM permissions;                       -- 期望 37
