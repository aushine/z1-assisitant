@echo off
REM ============================================================
REM  Z1 single-end release:  DESKTOP frontend only   (z1 -> /z1/)
REM
REM  Builds life-assisitant-ui-desktop, uploads dist/z1 to the Pi,
REM  swaps /var/www/z1 and reloads nginx. Touches NOTHING else:
REM  the mobile site (z1-app) and the API container are left
REM  exactly as they are. Use this when only the desktop UI changed.
REM
REM  Siblings:
...
echo ============================================================
echo   Z1 Release  [DESKTOP only]   build=%BUILD_VERSION%
echo   Pi: %PI_USER%@%PI_HOST%   web: %WEB_DIR%
echo ============================================================
echo.

REM ---------- 1. desktop UI ----------
echo [1/3] Building desktop UI ...
...
REM ---------- 2. upload ----------
echo [2/3] Uploading desktop static to Pi ...
...
ssh %PI_USER%@%PI_HOST% "sudo -n rm -rf %WEB_DIR%.bak; sudo -n cp -r %WEB_DIR% %WEB_DIR%.bak; sudo -n rm -rf %WEB_DIR% && sudo -n cp -r %PI_DIR%/dist/z1 %WEB_DIR% && sudo -n nginx -t && sudo -n systemctl reload nginx"
...
REM served index.html (catches the /z1/ base-path white-screen class).
echo Verifying ...
...
echo.
echo ============================================================
echo   RELEASE OK  [DESKTOP only]   build=%BUILD_VERSION%
